...
<body id="app">
    ...
    <script>
        ...
        // Global function to open cart modal
        window.openCartModal = function() {
            if (window.cartModalInstance) {
                window.cartModalInstance.open();
            } else {
                // Fallback: try to find the modal element and show it
                const modalElement = document.querySelector('[x-data="cartModal()"]');
                if (modalElement && modalElement.__x) {
                    modalElement.__x.$data.open();
                }
            }
        };
